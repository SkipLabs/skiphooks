...
// User mappings (GitHub → Slashwork username)

export interface DbUserMapping {
  githubUsername: string;
  slashworkUsername: string;
}

export async function getUserMappings(): Promise<DbUserMapping[]> {
  const result = await getPool().query<{
    github_username: string;
    slashwork_username: string;
  }>("SELECT github_username, slashwork_username FROM user_mappings ORDER BY github_username");
  return result.rows.map((row) => ({
    githubUsername: row.github_username,
    slashworkUsername: row.slashwork_username,
  }));
}

export async function getUserMappingsBatch(
  githubUsernames: string[],
): Promise<Map<string, string>> {
  if (githubUsernames.length === 0) return new Map();
  const lowered = githubUsernames.map((u) => u.toLowerCase());
  const result = await getPool().query<{
    github_username: string;
    slashwork_username: string;
  }>(
    "SELECT github_username, slashwork_username FROM user_mappings WHERE LOWER(github_username) = ANY($1)",
    [lowered],
  );
  const map = new Map<string, string>();
  for (const row of result.rows) {
    map.set(row.github_username.toLowerCase(), row.slashwork_username);
  }
  return map;
}
...
export async function upsertUserMapping(
  ...
  await getPool().query(
    `INSERT INTO user_mappings (github_username, slashwork_username)
     VALUES ($1, $2)
     ON CONFLICT (github_username) DO UPDATE SET slashwork_username = $2`,
    [githubUsername.toLowerCase(), slashworkUsername],
  );
}

export async function deleteUserMapping(githubUsername: string): Promise<boolean> {
  const result = await getPool().query(
    "DELETE FROM user_mappings WHERE LOWER(github_username) = LOWER($1)",
    [githubUsername],
  );
  return (result.rowCount ?? 0) > 0;
}
